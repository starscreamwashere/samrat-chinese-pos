...
import { LogOut, Plus, Minus, X } from "lucide-react";
...
export default function OrderPage() {
  ...
  function addToCart(item: MenuItem, size: Size) {
    const price =
      size === "half" ? item.halfPrice ?? item.fullPrice ?? 0 : item.fullPrice ?? 0;
    const gravy = false;
    const key = `${item.id}:${size}:${gravy ? "g" : "n"}`;
  ...
  function toggleGravy(line: CartLine) {
    ...
    setCart((prev) => {
      ...
      return [
        ...withoutOld,
        { ...line, key: newKey, gravy, unitPrice: newPrice },
      ];
    });
  }
      ...
      <div className="mx-2 rounded-t-2xl border border-black/10 bg-white shadow-pinned">
        ...
        {cart.length > 0 && expanded && (
          <div className="max-h-[35vh] divide-y divide-black/5 overflow-y-auto px-3 py-1">
            {cart.map((line) => {
              const starterLike =
                line.size !== "single"; // rice/noodles/starters have half/full
              return (
        ...
        <div className="border-t border-black/5 p-3">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {saving ? <Spinner className="text-white" /> : "Save Order"}
            </button>
          </div>
        </div>
      </div>
